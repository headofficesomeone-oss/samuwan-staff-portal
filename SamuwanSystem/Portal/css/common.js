const APP = {
  ...
  STORAGE_KEY: "samuwan_staff_session_v3"
};
...
function getSession() {
  try { return JSON.parse(localStorage.getItem(APP.STORAGE_KEY) || "null"); }
  catch (_) { return null; }
}
function saveSession(session) { localStorage.setItem(APP.STORAGE_KEY, JSON.stringify(session)); }
function clearSession() { localStorage.removeItem(APP.STORAGE_KEY); }
